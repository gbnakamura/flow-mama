...
export async function sendFlowMamaTestConfirmation(to: string) {
  const apiKey = process.env.RESEND_API_KEY;
  const from = process.env.RESEND_FROM_EMAIL;
  if (!apiKey || !from) throw new Error("Resend is not configured for this environment.");

  const sessions = [
    { dayLabel: "Mon", date: "21 Sept", fullDate: "Monday 21 September 2026", className: "Early Flow", time: "09:20–10:50" },
    { dayLabel: "Mon", date: "28 Sept", fullDate: "Monday 28 September 2026", className: "Early Flow", time: "09:20–10:50" },
  ];
  const total = "£44.00";
  const pricePerClass = "£22.00";
  const sessionRows = sessions.map((session) => `<tr><td style="padding:12px 0;border-top:1px solid #EAD9BE;font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#3A2E22;" width="60%"><strong>${session.dayLabel} ${session.date}</strong><br><span style="color:#7A6C5B;font-size:13px;">${session.className}</span></td><td align="right" style="padding:12px 0;border-top:1px solid #EAD9BE;font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#7A6C5B;" width="40%">${session.time}</td></tr>`).join("");

  const { error } = await new Resend(apiKey).emails.send({
    from,
    to,
    replyTo: process.env.BOOKING_ALERT_EMAIL ?? "amber@flowmamanorthfields.com",
    subject: "[Test] Your Flow Mama classes are confirmed",
    text: `Hi Guy,\n\nThis is a preview of the Flow Mama booking confirmation. I am very much looking forward to meeting you and Mia for our new term.\n\nYOUR SESSIONS\n${sessions.map((session) => `${session.fullDate} — ${session.className}, ${session.time}`).join("\n")}\n\n2 classes × ${pricePerClass}\nTotal paid: ${total}\n\nTHE VENUE\nRoom 2, Northfields Community Centre, W13 9SS\n\nWHAT TO BRING\n- A water bottle for yourself.\n- Your usual baby changing bag.\n- Optional: A familiar blanket or favourite toy.\n\nIMPORTANT: Complete the health screening form before your first session:\nhttps://docs.google.com/forms/d/e/1FAIpQLSfJZ7qgqo3uk3QVpmZK7oEdepXRIHfJ3tBSi8Hv9vbqOZzdQw/viewform?pli=1\n\nIf you have any questions at all, just hit reply. Otherwise, I will see you on the mat!\n\nWarmly,\nAmber`,
    html: `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Flow Mama confirmation preview</title><style>body,table,td{-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%}img{border:0;line-height:100%;outline:none;text-decoration:none}table{border-collapse:collapse!important}body{margin:0;padding:0;width:100%!important;background-color:#F8EFE0}@media screen and (max-width:600px){.email-container{width:100%!important}.px-fluid{padding-left:20px!important;padding-right:20px!important}}</style></head><body style="margin:0;padding:0;background-color:#F8EFE0;"><div style="display:none;max-height:0;overflow:hidden;opacity:0;mso-hide:all;">Test preview of the Flow Mama booking confirmation.</div><table role="presentation" width="100%" style="background-color:#F8EFE0;"><tr><td align="center" style="padding:32px 16px;"><table role="presentation" class="email-container" width="600" style="width:600px;max-width:600px;background-color:#FFFDF8;border-radius:16px;overflow:hidden;"><tr><td align="center" style="padding:32px 32px 20px;"><img src="https://www.flowmamanorthfields.com/images/logo.svg" width="160" height="51" alt="Flow Mama" style="display:block;width:160px;height:51px;margin:0 auto;"><div style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#7A6C5B;margin-top:4px;">Northfields Community Centre &middot; W13</div></td></tr><tr><td align="center" style="padding:0 32px 8px;"><table role="presentation"><tr><td style="background-color:#EAF0E6;color:#5F7A58;font-family:Arial,Helvetica,sans-serif;font-size:12px;font-weight:bold;padding:7px 16px;border-radius:999px;">&#10003; Test booking confirmed</td></tr></table></td></tr><tr><td class="px-fluid" style="padding:20px 40px 0;font-family:Georgia,'Times New Roman',serif;color:#3A2E22;"><p style="margin:0 0 16px;font-size:16px;line-height:1.6;">Hi Guy,</p><p style="margin:0 0 16px;font-size:16px;line-height:1.6;">This is a preview of the Flow Mama booking confirmation. I am very much looking forward to meeting you and Mia for our new term.</p><p style="margin:0;font-size:16px;line-height:1.6;font-weight:bold;">Here is everything you need to know ahead of your first session:</p></td></tr><tr><td class="px-fluid" style="padding:20px 40px 4px;"><table role="presentation" width="100%" style="background-color:#F8EFE0;border-radius:12px;"><tr><td style="padding:20px 22px;font-family:Arial,Helvetica,sans-serif;color:#3A2E22;"><div style="font-size:11px;font-weight:bold;letter-spacing:.06em;text-transform:uppercase;color:#7A6C5B;margin-bottom:8px;">📍 The Venue</div><div style="font-size:14.5px;line-height:1.6;"><strong>Room 2, Northfields Community Centre, W13 9SS.</strong> <a href="https://www.google.com/maps/search/?api=1&amp;query=Northfields%20Community%20Centre%2C%20W13%209SS" style="color:#E2734A;text-decoration:none;">Open in Google Maps</a><br>There is a covered buggy park outside and limited parking spaces.</div><div style="border-top:1px solid #EAD9BE;margin-top:18px;padding-top:18px;"><div style="font-size:14.5px;font-weight:bold;margin-bottom:8px;">🎒 What to bring:</div><ul style="margin:0;padding-left:20px;font-size:14.5px;line-height:1.6;"><li>A water bottle for yourself.</li><li>Your usual baby changing bag.</li><li>Optional: a familiar blanket or favourite toy.</li></ul><div style="font-size:14.5px;line-height:1.6;margin-top:12px;"><strong>Note:</strong> Yoga mats and props are provided.</div></div><div style="border-top:1px solid #EAD9BE;margin-top:18px;padding-top:18px;"><div style="font-size:14.5px;font-weight:bold;margin-bottom:8px;">📋 IMPORTANT: Your Health Screening Form</div><div style="font-size:14.5px;line-height:1.6;">Please complete the <a href="https://docs.google.com/forms/d/e/1FAIpQLSfJZ7qgqo3uk3QVpmZK7oEdepXRIHfJ3tBSi8Hv9vbqOZzdQw/viewform?pli=1" style="color:#E2734A;font-weight:bold;text-decoration:underline;">health questionnaire</a> before your first session.</div></div></td></tr></table></td></tr><tr><td class="px-fluid" style="padding:28px 40px 0;"><div style="font-family:Georgia,'Times New Roman',serif;font-weight:bold;font-size:16px;color:#3A2E22;margin-bottom:4px;">Your sessions this term</div><div style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#7A6C5B;margin-bottom:14px;">2 classes &middot; through 28 Sept</div><table role="presentation" width="100%">${sessionRows}</table></td></tr><tr><td class="px-fluid" style="padding:20px 40px 0;"><table role="presentation" width="100%" style="border-top:2px solid #3A2E22;"><tr><td style="padding:14px 0 4px;font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#7A6C5B;">2 classes &times; ${pricePerClass}</td><td align="right" style="padding:14px 0 4px;font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#7A6C5B;">${total}</td></tr><tr><td style="padding:2px 0 16px;font-family:Georgia,'Times New Roman',serif;font-size:17px;font-weight:bold;color:#3A2E22;">Total paid</td><td align="right" style="padding:2px 0 16px;font-family:Georgia,'Times New Roman',serif;font-size:17px;font-weight:bold;color:#3A2E22;">${total}</td></tr></table></td></tr><tr><td class="px-fluid" style="padding:8px 40px 28px;font-family:Georgia,'Times New Roman',serif;color:#3A2E22;"><p style="margin:0 0 14px;font-size:15px;line-height:1.6;">If you have any questions at all, just hit reply. Otherwise, I will see you on the mat!</p><p style="margin:0;font-size:15px;line-height:1.6;">Warmly,<br><strong>Amber</strong></p></td></tr></table></td></tr></table></body></html>`,
  });
  if (error) throw new Error(`Resend rejected the Flow Mama preview: ${error.message}`);
}
...
export async function sendPersonalTrainingTestConfirmation(to: string) {
  const apiKey = process.env.RESEND_API_KEY;
  const from = process.env.RESEND_FROM_EMAIL;
  if (!apiKey || !from) throw new Error("Resend is not configured for this environment.");

  const sessions = [
    { dayLabel: "Fri", date: "25 Sept", fullDate: "Friday 25 September 2026", time: "09:00–10:00", label: "Group Personal Training", price: 30 },
    { dayLabel: "Fri", date: "2 Oct", fullDate: "Friday 2 October 2026", time: "10:00–11:00", label: "1:1 Personal Training", price: 50 },
  ];
  const total = moneyFormatter.format(sessions.reduce((sum, session) => sum + session.price, 0));
  const sessionRows = sessions.map((session) => `
    <tr>
      <td style="padding:12px 0;border-top:1px solid #EAD9BE;font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#3A2E22;" width="52%">
        <strong>${escapeHtml(session.dayLabel)} ${escapeHtml(session.date)}</strong><br>
      ...
      <td align="right" style="padding:12px 0;border-top:1px solid #EAD9BE;font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#7A6C5B;" width="30%">${escapeHtml(session.time)}</td>
      <td align="right" style="padding:12px 0;border-top:1px solid #EAD9BE;font-family:Arial,Helvetica,sans-serif;font-size:14px;font-weight:bold;color:#3A2E22;" width="18%">${escapeHtml(moneyFormatter.format(session.price))}</td>
    </tr>
  `).join("");
  const plainTextSessions = sessions.map((session) => `${session.fullDate} — ${session.time} · ${session.label} · ${moneyFormatter.format(session.price)}`).join("\n");

  const { error } = await new Resend(apiKey).emails.send({
    from,
    to,
    replyTo: process.env.BOOKING_ALERT_EMAIL ?? "amber@flowmamanorthfields.com",
    subject: "[Test] Your personal training sessions are confirmed",
    text: `Hi Guy,\n\nThis is a preview of the Personal Training booking confirmation.\n\nYOUR SESSIONS\n${plainTextSessions}\n\n${sessions.length} sessions\nTotal paid: ${total}\n\nWarmly,\nAmber\n\nQuestions about your booking? Just reply to this email.`,
    html: `
      <!doctype html>
      <html lang="en">
      <head>
        ...
        <title>Personal training confirmation preview</title>
        <style>body,table,td{-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%}img{border:0;line-height:100%;outline:none;text-decoration:none}table{border-collapse:collapse!important}body{margin:0;padding:0;width:100%!important;background-color:#F8EFE0}@media screen and (max-width:600px){.email-container{width:100%!important}.px-fluid{padding-left:20px!important;padding-right:20px!important}}</style>
      </head>
      <body style="margin:0;padding:0;background-color:#F8EFE0;">
        <div style="display:none;max-height:0;overflow:hidden;opacity:0;mso-hide:all;">Test preview of the Personal Training booking confirmation.</div>
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#F8EFE0;"><tr><td align="center" style="padding:32px 16px;">
          <table role="presentation" class="email-container" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px;max-width:600px;background-color:#FFFDF8;border-radius:16px;overflow:hidden;">
            <tr><td align="center" style="padding:32px 32px 20px;"><img src="https://www.flowmamanorthfields.com/images/logo.svg" width="160" height="51" alt="Flow Mama" style="display:block;width:160px;height:51px;margin:0 auto;"><div style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#7A6C5B;margin-top:4px;">Personal Training &middot; Northfields</div></td></tr>
            <tr><td align="center" style="padding:0 32px 8px;"><table role="presentation"><tr><td style="background-color:#EAF0E6;color:#5F7A58;font-family:Arial,Helvetica,sans-serif;font-size:12px;font-weight:bold;padding:7px 16px;border-radius:999px;">&#10003; Test booking confirmed</td></tr></table></td></tr>
            <tr><td class="px-fluid" style="padding:20px 40px 0;font-family:Georgia,'Times New Roman',serif;color:#3A2E22;"><p style="margin:0 0 16px;font-size:16px;line-height:1.6;">Hi Guy,</p><p style="margin:0;font-size:16px;line-height:1.6;">This is a preview of the Personal Training booking confirmation.</p></td></tr>
            <tr><td class="px-fluid" style="padding:28px 40px 0;"><div style="font-family:Georgia,'Times New Roman',serif;font-weight:bold;font-size:16px;color:#3A2E22;margin-bottom:4px;">Your sessions</div><div style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#7A6C5B;margin-bottom:14px;">${sessions.length} sessions</div><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">${sessionRows}</table></td></tr>
            <tr><td class="px-fluid" style="padding:20px 40px 0;"><table role="presentation" width="100%" style="border-top:2px solid #3A2E22;"><tr><td style="padding:14px 0 18px;font-family:Georgia,'Times New Roman',serif;font-size:17px;font-weight:bold;color:#3A2E22;">Total paid</td><td align="right" style="padding:14px 0 18px;font-family:Georgia,'Times New Roman',serif;font-size:17px;font-weight:bold;color:#3A2E22;">${escapeHtml(total)}</td></tr></table></td></tr>
            <tr><td class="px-fluid" style="padding:8px 40px 28px;font-family:Georgia,'Times New Roman',serif;color:#3A2E22;"><p style="margin:0;font-size:15px;line-height:1.6;">Warmly,<br><strong>Amber</strong></p></td></tr>
            <tr><td style="padding:22px 40px;background-color:#F8EFE0;font-family:Arial,Helvetica,sans-serif;"><p style="margin:0;font-size:12.5px;color:#7A6C5B;text-align:center;">Questions about your booking? Just reply to this email.</p></td></tr>
          </table>
        </td></tr></table>
      </body></html>`,
  });
  if (error) throw new Error(`Resend rejected the Personal Training preview: ${error.message}`);
}
